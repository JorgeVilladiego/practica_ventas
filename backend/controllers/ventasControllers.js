...
export const registrarVenta = (reg, res) => {
    const venta = req.body;
    if (!venta.id_cliente || !venta.id_producto || !venta.cantidad){
        return res.status(400).json({ error: 'Faltan campos requeridos'});
    }
    registrarVentaModel(venta, (err, resultado) => {
        if (err) return res.estatus(500).json({ error: err.message});
        res.status(201).json({ mensaje: 'venta regustrada exitosamente', id: resultado.insertId});
    });
};
